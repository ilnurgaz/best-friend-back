...
router.post('/create', async (req, res) => {
    ...
    const baseUrl = 'https://bestfriend-back.onrender.com/uploads/';
...
router.get('/all/bear', async (req, res) => {
    try {
        // Только записи, где voice существует и равно 'm'
        const audios = await Audio.find({ voice: 'b' });
        res.json(audios);
    } catch (err) {
        res.status(500).send(`Error fetching audio information: ${err.message}`);
    }
...
router.get('/all/rabbit', async (req, res) => {
    try {
        // Только записи, где voice существует и равно 'b'
        const audios = await Audio.find({ voice: 'r' });
        res.json(audios);
    } catch (err) {
        res.status(500).send(`Error fetching audio information: ${err.message}`);
    }
